...
import elong.CrazyZomby.R;
import elong.CrazyZomby.Control.CtlDisappear;
import elong.CrazyZomby.Control.CtlExchange;
import elong.CrazyZomby.Control.CtlLifeAdd;
import elong.CrazyZomby.Control.CtlLifeDel;
import elong.CrazyZomby.Control.CtlMonster;
import elong.CrazyZomby.Control.CtlResultScore;
import elong.CrazyZomby.Control.CtlTip1;
import elong.CrazyZomby.Control.CtlTip2;
import elong.CrazyZomby.CrazyZombyConstent.E_SOUND;
import elong.CrazyZomby.CrazyZombyConstent.E_TIP;
import elong.CrazyZomby.Draw.DrawAnimal;
import elong.CrazyZomby.Draw.DrawAutoTip;
import elong.CrazyZomby.Draw.DrawBomb;
import elong.CrazyZomby.Draw.DrawBackGround;
import elong.CrazyZomby.Draw.DrawExplosion;
import elong.CrazyZomby.Draw.DrawLife;
import elong.CrazyZomby.Draw.DrawLifeAdd;
import elong.CrazyZomby.Draw.DrawLifeDel;
import elong.CrazyZomby.Draw.DrawMenu;
import elong.CrazyZomby.Draw.DrawMonster;
import elong.CrazyZomby.Draw.DrawResultScore;
import elong.CrazyZomby.Draw.DrawSingleScore;
import elong.CrazyZomby.Draw.DrawTimeBar;
import elong.CrazyZomby.Draw.DrawTip1;
import elong.CrazyZomby.Draw.DrawDisappear;
import elong.CrazyZomby.Draw.DrawExchange;
import elong.CrazyZomby.Draw.DrawFill;
import elong.CrazyZomby.Draw.DrawGrid;
import elong.CrazyZomby.Draw.DrawLoading;
import elong.CrazyZomby.Draw.DrawScore;
import elong.CrazyZomby.Draw.DrawTip2;
import elong.CrazyZomby.Interaction.ScreenTouch;
import elong.CrazyZomby.Interaction.TouchMsg;
...
public class ControlCenter {
	...
	public static E_GAMEMODE mMode = E_GAMEMODE.LIFE;
	...
	//menu
	int menuTextureId;
	int timeModePicTextureId;
	int lifeModePicTextureId;
	int timeModeTxtTextureId;
	int lifeModeTxtTextureId;
    ...
    static public DrawMenu drawMenuTimeModePic;
    static public DrawMenu drawMenuLifeModePic;
    static public DrawMenu drawMenuTimeModeTxt;
    static public DrawMenu drawMenuLifeModeTxt;
    
    //result
    static public DrawBackGround drawResultBackGround;
	static public DrawResultScore drawResultScore;
	...
	public static Life mLife;			//生命数
	public static Sound mSound;			//声音效果对象
	public static Timer mTimer;			//计时器对象
	public static ScreenTouch mScreenTouch;
	public static TouchMsg mTouchMsg;	//触摸消息
	...
	public ControlCenter(Context context)
	{
		...
		mTimer = new Timer(CrazyZombyConstent.MAX_TIME);
	    mAnimalPic = new int[(int) CrazyZombyConstent.GRID_NUM][(int) CrazyZombyConstent.GRID_NUM];
	    mPicBak = new int[(int) CrazyZombyConstent.GRID_NUM][(int) CrazyZombyConstent.GRID_NUM];
	    mEffect = new int[(int) CrazyZombyConstent.GRID_NUM][(int) CrazyZombyConstent.GRID_NUM];
	    mDisappearToken = new int[(int) CrazyZombyConstent.GRID_NUM][(int) CrazyZombyConstent.GRID_NUM];
	    mToken = new ActionTokenPool();
		mTouchMsg = new TouchMsg();    
		init();
		//startMenuScene();
	}
	...
	static void fillGrid(int col, int row)
	{
		...
		{
			for(int i = row; i < (int)CrazyZombyConstent.GRID_NUM; i++)
			{
	...
	static void fillMethod()
	{
		for(int i = 0; i < (int)CrazyZombyConstent.GRID_NUM; i++)
		{
			for(int j = 0; j < (int)CrazyZombyConstent.GRID_NUM; j++) 
			{
				...
				{
					if(j < (int)CrazyZombyConstent.GRID_NUM - 1)
					{
						//从上一行中跌落
    ...
    {	
		...
		for(int i = 1; i < (int)CrazyZombyConstent.GRID_NUM - 1; i++)
		{
			for(int j = 1; j < (int)CrazyZombyConstent.GRID_NUM - 1; j++) 
			{
	...
	static void markBomb(int token, int col, int row)
	{
		...
		if (isBomb(col,row))
		{
			...
			{
				if(i >= 0 && i < (int)CrazyZombyConstent.GRID_NUM)
				{
					for(int j = row-1; j <= row+1; j++)
					{
						if(j >= 0 && j < (int)CrazyZombyConstent.GRID_NUM)
						{
	...
	static void genSpecialAnimal()
	{
		...
		data = (data % ((int)CrazyZombyConstent.GRID_NUM * (int)CrazyZombyConstent.GRID_NUM));
		int x = 0;
		int y = 0;
		x = data / (int)CrazyZombyConstent.GRID_NUM;
		y = data % (int)CrazyZombyConstent.GRID_NUM;
	...
	public void initTexture(GL10 gl)
	{
		...
		timeModePicTextureId = initTexture(gl, R.drawable.action1);
		lifeModePicTextureId = initTexture(gl, R.drawable.action2);
		timeModeTxtTextureId = initTexture(gl, R.drawable.timemode);
		lifeModeTxtTextureId = initTexture(gl, R.drawable.lifemode);
	...
	void initControl()
	{
    	...
    	controlRegister(mLife);				//生命的超时控制
    	
    	//menu
    	controlRegister(drawMenuTimeModePic.control);
    	controlRegister(drawMenuLifeModePic.control);
	...
	public void initDraw(GL10 gl)
	{
		...
		drawMenuLifeModePic = new DrawMenu(lifeModePicTextureId, 1, 0, 128, 128, 4, E_TOUCHAREA.MENU_LIFE_MODE);
		drawMenuTimeModeTxt = new DrawMenu(timeModeTxtTextureId, 3, 3, 256, 128, 1, E_TOUCHAREA.MENU_TIME_MODE);
		drawMenuLifeModeTxt = new DrawMenu(lifeModeTxtTextureId, 3, 0, 256, 128, 1, E_TOUCHAREA.MENU_LIFE_MODE);

		//result
		drawResultBackGround = new DrawBackGround(resultTextureId);
		drawResultScore = new DrawResultScore(scoreTextureId);
		drawTryAgain = new DrawMenu(tryAgainTextureId, 3, 1, 256, 256, 4, E_TOUCHAREA.RESULT_CONTINUE);
    	...
    	drawTimeBar = new DrawTimeBar(gl, timeBarTextureId, CrazyZombyConstent.MAX_TIME);
	...
	public static final int RESULT_END = 17;
	
	//预留的TOUCH事件处理，注意TOUCH_EVENT_BASE ~ (TOUCH_EVENT_BASE + E_TOUCHAREA.MAX)保证不能与其他冲突
	public static final int TOUCH_EVENT_BASE = 100;
    ...
    public static Handler mHandler = new Handler(){   
		...
		public void handleMessage(Message msg) 
		{
        	...
        	if(msg.what >= TOUCH_EVENT_BASE && msg.what < TOUCH_EVENT_BASE + E_TOUCHAREA.MAX.ordinal())
        	{
        		dealTouchMsg(msg.what);
        	}
        	else
        	{
    			switch(msg.what)
    			{
    				...
    				case GAME_OVER_END:
    				{
    					...
    					CtlResultScore ctl = (CtlResultScore)drawResultScore.control;
    					ctl.init(mScore.getScore());
    					mSound.play(E_SOUND.RESULTSCORE);
    					stopResultScene();
    					break;
    				}
    				case LIFEADD_START:
    ...
    {
    	mAutoTipTimer++;
    	if(mAutoTipTimer > CrazyZombyConstent.AUTOTIP_DELAY)
    	{
    		if(!mIsAutoTip)
    		{
    			mIsAutoTip = true;
    ...
    {
    	drawMenuTimeModePic.control.start();
    	drawMenuLifeModePic.control.start();
    	drawMenuTimeModeTxt.control.start();
    	drawMenuLifeModeTxt.control.start();
    }

    static void stopMenuScene()
    {
    	drawMenuTimeModePic.control.end();
    	drawMenuLifeModePic.control.end();
    	drawMenuTimeModeTxt.control.end();
    	drawMenuLifeModeTxt.control.end();
    }
    ...
    static void stopResultScene()
    {
    	drawTryAgain.control.end();
    }

    //菜单场景渲染
    public void drawMenuScene(GL10 gl)
    {
    	drawMenuTimeModePic.draw(gl);
    	drawMenuLifeModePic.draw(gl);
    	drawMenuTimeModeTxt.draw(gl);
    	drawMenuLifeModeTxt.draw(gl);
    ...
    {
    	drawResultScore.draw(gl);
    	drawTryAgain.draw(gl);
	...
	public void drawGameScene(GL10 gl)
	{
		...
		drawTip1.draw(gl);
		drawTip2.draw(gl);
		if(mMode == E_GAMEMODE.LIFE)
		{
			drawLife.draw(gl, mLife.get());
			drawLifeAdd.draw(gl);
			drawLifeDel.draw(gl);
		}
		else if(mMode == E_GAMEMODE.TIME)
		{
