...
public class ScreenTouch {
	...
	public boolean touchMenuView(MotionEvent e) {
		switch (e.getAction()) {		
			case MotionEvent.ACTION_UP:
				raiseTouchMenuViewEvent(e.getX(), e.getY());
				break;
		}
		return true;
	}
	...
	public boolean touchResultView(MotionEvent e) {
		switch (e.getAction()) {		
		case MotionEvent.ACTION_UP:
			raiseTouchResultViewEvent(e.getX(), e.getY());
			break;
		}
		return true;		
	}
	...
	public boolean touchGameView(MotionEvent e) {
		...
		switch (e.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
			    ...
			    {
			    	if(dy > CrazyZombyConstent.MOVE_THRESDHOLDER) mMoveDirection = TOUCH_DIRECTION.DOWN;
			    	else if(dy < -CrazyZombyConstent.MOVE_THRESDHOLDER) mMoveDirection = TOUCH_DIRECTION.UP;
			    }
			    else
			    {
			    	if(dx > CrazyZombyConstent.MOVE_THRESDHOLDER) mMoveDirection = TOUCH_DIRECTION.RIGHT;
			    	else if(dx < -CrazyZombyConstent.MOVE_THRESDHOLDER) mMoveDirection = TOUCH_DIRECTION.LEFT; 	
			    }
		}
	...
	void raiseTouchMenuViewEvent(float x, float y)
	{
		ControlCenter.mTouchMsg.raiseTouchEvent((int)x, (int)y);
	}

	void raiseTouchResultViewEvent(float x, float y)
	{
		ControlCenter.mTouchMsg.raiseTouchEvent((int)x, (int)y);
	}
	...
	void raiseTouchGameViewEvent()
	{
		...
		int x  = getGridX();
		int y = getGridY();
		if(!(x >= 0 && x < (int)CrazyZombyConstent.GRID_NUM)) return;
		if(!(y >= 0 && y < (int)CrazyZombyConstent.GRID_NUM)) return;
