...
public class DrawTip1 {
    ...
    private void initVertexBuffer()
    {
    	...
    	int w = CrazyZombyConstent.ADP_SIZE*ctl.getW();
    	int h = CrazyZombyConstent.ADP_SIZE*ctl.getH();
    	float x = ctl.getX()/10.0f;
    	float y = ctl.getY()/10.0f;
    	
        vCount=6;//顶点的数量，一个正方形用两个三角形表示，共需要6个顶点   
        int deltaX = (int)(-(2.0f-x)*64*CrazyZombyConstent.ADP_SIZE);
        int deltaY = (int)((4.8f-y)*2*32*CrazyZombyConstent.ADP_SIZE);
